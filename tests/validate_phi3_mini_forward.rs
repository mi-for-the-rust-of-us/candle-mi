...
//! Integration test: `microsoft/Phi-3-mini-4k-instruct` forward-pass parity
//! against the from-first-principles Python oracle in
//! `scripts/phi3_mini_validation.py`.
//!
//! Phi-3's distinguishing trait is **fused projections**: a single `qkv_proj`
//! (split into Q/K/V via `narrow`) and a single `gate_up_proj` (split into the
//! `SwiGLU` gate/up halves).  Its only prior guard was a "`Paris` in top-5"
//! smoke test in `validate_models.rs`; this is the exact-logit guard.
//!
//! Consumes the frozen reference JSON
//! (`scripts/phi3_mini_forward_reference.json`) and verifies that candle-mi's
//! [`GenericTransformer`] produces matching output when fed the same input
//! prompts.  Acceptance bar:
//!
//! - Detected config has `qkv_layout == Fused` and `mlp_layout == GatedFused`.
...
//!
//! Requires `microsoft/Phi-3-mini-4k-instruct` cached in
//! `~/.cache/huggingface/hub/`.
//!
//! Run CPU:
//!   `cargo test --test validate_phi3_mini_forward --features transformer -- --ignored phi3_mini_forward_parity_cpu`
//!
//! Run GPU:
//!   `cargo test --test validate_phi3_mini_forward --features transformer -- --ignored phi3_mini_forward_parity_gpu`
...
/// Run the Phi-3 forward-parity check on the given `device`.  Prints a
...
fn run_phi3_mini_forward_parity(device: &Device, device_name: &str, abs_diff_bar: f32) {
    let reference_str = std::fs::read_to_string(reference_path("phi3_mini_forward_reference.json")).expect(
    ...
    // Phi-3's distinguishing traits: fused QKV + fused gate-up MLP.
    assert_eq!(
        config.qkv_layout,
        QkvLayout::Fused,
