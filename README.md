...
| **Self-conditioning** *(new in v0.2.0)* | A masked-diffusion model can be given its own previous prediction as a second input, so it can revise a draft instead of committing one position at a time irrevocably (Chen, Zhang & Hinton 2022). `OthelloGpt::forward_with_self_cond` takes an optional token grid; the embedding is added before the first hook fires, so the logit lens and every capture read the stream the logits came from. Off by default, and with it off the forward is bit-identical to a model built without it | [the ask, and the crate's closure](docs/dogfooding-feedbacks/othello-mdlm-needs-a-carry-channel.md) |
