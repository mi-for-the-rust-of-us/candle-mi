...
|---|---|---|---|
| [transformers-gemma1-activation-regression.md](transformers-gemma1-activation-regression.md) | `transformers` | bug report | **DRAFT, not filed.** First document here aimed outside `huggingface/candle`. Since v4.48.0 (#35235, 2024-12-18) `transformers` computes exact erf GELU for the five Gemma 1.0 checkpoints instead of `gelu_pytorch_tanh`, silently: the same diff that replaced `GemmaMLP`'s `hidden_activation` guard with `ACT2FN[config.hidden_act]` deleted its warning. Found by `tests/validate_gemma_forward.rs`. Measured inside `transformers` alone, swapping only `mlp.act_fn` on one loaded model: max abs-diff `1.978e-2` over 30 top-10 logits with **0 of 30** index mismatches, which is why no eval or smoke test would catch it. candle-mi is unaffected (`parse_gemma` hardcodes `GeluApprox`, now with a comment saying why) |
